...
*Setup instructions for a Gitian build of QtumLite Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the QtumLite
...
- [Building QtumLite Core](#building-qtumlite-core)
...
Clone the git repositories for qtumlite and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/qtumliteproject/qtumlite --recursive
...
```bash
sudo cp /home/debian/qtumlite/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the qtumlite repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building QtumLite Core
----------------

To build QtumLite Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the qtumlite repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/qtumlite/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/qtumliteproject/qtumlite
...
For example:
```bash
URL=https://github.com/laanwj/qtumlite.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit qtumlite=${COMMIT} --url qtumlite=${URL} ../qtumlite/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit qtumlite=${COMMIT} --url qtumlite=${URL} ../qtumlite/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit qtumlite=${COMMIT} --url qtumlite=${URL} ../qtumlite/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-qtumlite:
./bin/gbuild --commit qtumlite=${COMMIT},cpp-eth-qtumlite=${ETHCOMMIT} --url qtumlite=${URL},cpp-eth-qtumlite=${ETHURL} ../qtumlite/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the qtumlite git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../qtumlite/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/qtumproject/qtumlite-detached-sigs.git

BTCPATH=/some/root/path/qtumlite
SIGPATH=/some/root/path/qtumlite-detached-sigs

./bin/gbuild --url qtumlite=${BTCPATH},signature=${SIGPATH} ../qtumlite/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/qtumlite-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/qtumlite-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/qtumlite-osx-build.assert
```
...
[qtumliteproject/gitian.sigs](https://github.com/qtumliteproject/gitian.sigs/) repository.
